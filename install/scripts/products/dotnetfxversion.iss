[Code]
type
  NetFXType = (NetFx10, NetFx11, NetFx20, NetFx30, NetFx35, NetFx40Client, NetFx40Full, NetFx45Full, NetFx4x);
  ...
  case version of
    ...
    NetFx45Full:
      if (not RegQueryDWordValue(HKLM, netfx11plus_reg + 'v4\Full' + lcid, 'Servicing', regVersion)) then
        regVersion := -1;
    NetFx4x:
      // https://msdn.microsoft.com/pl-pl/library/bb822049(v=vs.110).aspx
      if (RegQueryDWordValue(HKLM, netfx11plus_reg + 'v4\Full' + lcid, 'Release', regVersion)) then begin
      Log('regVersion=' + IntToStr(regVersion));
				if (regVersion>= 394802) then 
					regVersion := 62 // 4.6.2+
				else if (regVersion >= 394254) 
					then regVersion := 61 // 4.6.1
